...
int32_t mz_os_make_symlink(const char *path, const char *target_path) {
    ...
    VirtualQuery((void *)VirtualQuery, &mbi, sizeof(mbi));
...
int32_t mz_os_read_symlink(const char *path, char *target_path, int32_t max_target_path) {
    ...
    if (DeviceIoControl(handle, FSCTL_GET_REPARSE_POINT, NULL, 0, buffer, sizeof(buffer), &length, NULL) == TRUE) {
        ...
        if ((IsReparseTagMicrosoft(reparse_data->ReparseTag)) &&
            ...
            if (target_path_len > reparse_data->DUMMYUNION.SymbolicLinkReparseBuffer.PrintNameLength)
                target_path_len = reparse_data->DUMMYUNION.SymbolicLinkReparseBuffer.PrintNameLength;

            target_path_wide = (wchar_t *)malloc(target_path_len + sizeof(wchar_t));
            if (target_path_wide) {
                target_path_idx = reparse_data->DUMMYUNION.SymbolicLinkReparseBuffer.PrintNameOffset / sizeof(wchar_t);
                memcpy(target_path_wide, &reparse_data->DUMMYUNION.SymbolicLinkReparseBuffer.PathBuffer[target_path_idx],
                    target_path_len);
