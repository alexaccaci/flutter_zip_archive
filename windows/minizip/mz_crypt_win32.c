...
void mz_crypt_sha_reset(void *handle) {
    ...
    if (sha->algorithm == MZ_HASH_SHA224) {
        free(sha->DUMMYUNION.sha224);
        sha->DUMMYUNION.sha224 = NULL;
    } else {
        if (sha->DUMMYUNION.DUMMYSTRUCT.hash)
            CryptDestroyHash(sha->DUMMYUNION.DUMMYSTRUCT.hash);
        sha->DUMMYUNION.DUMMYSTRUCT.hash = 0;
        if (sha->DUMMYUNION.DUMMYSTRUCT.provider)
            CryptReleaseContext(sha->DUMMYUNION.DUMMYSTRUCT.provider, 0);
        sha->DUMMYUNION.DUMMYSTRUCT.provider = 0;
    }
    sha->error = 0;
}
...
int32_t mz_crypt_sha_begin(void *handle) {
    ...
    if (sha->algorithm == MZ_HASH_SHA224) {
        sha->DUMMYUNION.sha224 = malloc(sizeof(mz_crypt_sha224));
        if (!sha->DUMMYUNION.sha224)
            return MZ_MEM_ERROR;
        mz_crypt_sha224_init(sha->DUMMYUNION.sha224);
        return MZ_OK;
    }
    ...
    result = CryptAcquireContext(&sha->DUMMYUNION.DUMMYSTRUCT.provider, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT | CRYPT_SILENT);
    if (!result) {
    ...
    if (result) {
        result = CryptCreateHash(sha->DUMMYUNION.DUMMYSTRUCT.provider, alg_id, 0, 0, &sha->DUMMYUNION.DUMMYSTRUCT.hash);
        if (!result) {
            sha->error = GetLastError();
            err = MZ_HASH_ERROR;
        }
...
int32_t mz_crypt_sha_update(void *handle, const void *buf, int32_t size) {
    ...
    if (sha->algorithm == MZ_HASH_SHA224) {
        if (!sha->DUMMYUNION.sha224)
            return MZ_PARAM_ERROR;
        mz_crypt_sha224_update(sha->DUMMYUNION.sha224, buf, size);
        return size;
    }

    if (sha->DUMMYUNION.DUMMYSTRUCT.hash == 0)
        return MZ_PARAM_ERROR;

    result = CryptHashData(sha->DUMMYUNION.DUMMYSTRUCT.hash, buf, size, 0);
    if (!result) {
        sha->error = GetLastError();
...
int32_t mz_crypt_sha_end(void *handle, uint8_t *digest, int32_t digest_size) {
    ...
    if (sha->algorithm == MZ_HASH_SHA224) {
        if (!sha->DUMMYUNION.sha224 || digest_size < 28)
            return MZ_PARAM_ERROR;
        mz_crypt_sha224_end(sha->DUMMYUNION.sha224, digest);
        return MZ_OK;
    }
    ...
    result = CryptGetHashParam(sha->DUMMYUNION.DUMMYSTRUCT.hash, HP_HASHVAL, NULL, (DWORD *)&expected_size, 0);
    if (expected_size > digest_size)
        return MZ_BUF_ERROR;
    if (!result)
        return MZ_HASH_ERROR;
    result = CryptGetHashParam(sha->DUMMYUNION.DUMMYSTRUCT.hash, HP_HASHVAL, digest, (DWORD *)&digest_size, 0);
    if (!result) {
        sha->error = GetLastError();
        return MZ_HASH_ERROR;
    }
